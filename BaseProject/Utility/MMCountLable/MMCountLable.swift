...
public class MMCountLable: UILabel {
    ...
    internal let dateCur = NSDate(timeIntervalSinceNow: 0)
    ...
    public var timeFormat = "dd" //"dd:hh:mm:ss"
    ...
    public func setCountDownDate(targetDate: NSDate) {
        ...
        let start = Date()
//        let StartTime:NSDate = start.toLocalTime() as NSDate
//
//        print("Current DATE:-\(StartTime)")
//        setCountDownDate(fromDate: StartTime, targetDate: targetDate)
    }
...
extension MMCountLable {
    func setup() {
//        self.font = UIFont.boldLabelFontOfSize(size: 12.0)
    }
    ...
    func updateText() {
        ...
        if let countdownAttributedText = countdownAttributedText {
            ...
            if TypeEventLable == 1{
                text =  "Event ends in \(attributedString.string)"
            }
            else if TypeEventLable == 2{
                text =  "Event starts in \(attributedString.string)"
            }
            else{
                text = "\(attributedString.string) Days to go"
            }
            
        } else {
            
            if TypeEventLable == 1{
                text =  "Event ends in \(formattedText)"
            }
            else if TypeEventLable == 2{
                text =  "Event starts in \(formattedText)"
            }
            else{
                text = "\(formattedText) Days to go"
            }
    ...
    func surplusTime(_ to1970Date: Date) -> String {
        ...
        if let day = comp.day ,let _ = timeFormat.range(of: "dd"){
            if day != 0{
                ...
                labelText = labelText.replacingOccurrences(of: "dd:", with: String.init(format: ""))
            }
        }
        if let hour = comp.hour ,let _ = timeFormat.range(of: "hh"){
            if hour != 0 || comp.day != 0{
                if TypeEventLable == 3 {
                    labelText =  ""
                }
                labelText = labelText.replacingOccurrences(of: "hh", with: String.init(format: "%02ld", hour))
            }else{
                labelText = labelText.replacingOccurrences(of: "hh:", with: String.init(format: ""))
            }
        }
        if let hour = comp.hour ,let _ = timeFormat.range(of: "HH"){
            if hour != 0{
